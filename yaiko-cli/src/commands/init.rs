...
pub async fn run(name: &str, database: &str) -> anyhow::Result<()> {
    ...
    generate_cargo_toml(project_path, name, database)?;
    pb.inc(1);
...
fn create_directories(project_path: &Path) -> anyhow::Result<()> {
    let dirs = [
        "", "src", "src/controllers", "src/models", "src/middleware",
        "public", "public/css", "public/js", "public/images", "public/fonts",
        "templates", "templates/layouts", "templates/partials",
        "migrations", "config",
    ];
    for dir in dirs {
...
fn generate_source_files(project_path: &Path) -> anyhow::Result<()> {
    ...
    fs::write(project_path.join("src/controllers/mod.rs"), "pub mod users;\n")?;
    fs::write(project_path.join("src/controllers/users.rs"), include_str!("../templates/users.rs.tmpl"))?;
    fs::write(project_path.join("src/models/mod.rs"), "// Models\n")?;
    fs::write(project_path.join("src/middleware/mod.rs"), "// Middleware\n")?;
    Ok(())
}

fn generate_frontend_assets(project_path: &Path) -> anyhow::Result<()> {
    fs::write(project_path.join("templates/index.html"), include_str!("../templates/index.html.tmpl"))?;
    fs::write(project_path.join("public/css/main.css"), include_str!("../templates/main.css.tmpl"))?;
    fs::write(project_path.join("public/js/core.js"), include_str!("../templates/core.js.tmpl"))?;
    fs::write(project_path.join("public/js/app.js"), include_str!("../templates/app.js.tmpl"))?;
    Ok(())
}
